...
0){if(show_replace==1){out+='<div id="tipue_search_warning_head">Показаны результаты для '+d+"</div>";out+='<div id="tipue_search_warning">Поиск для <a href="javascript:void(0)" id="tipue_search_replaced">'+d_r+"</a></div>"}if(c==1)out+='<div id="tipue_search_results_count">1 результат</div>';else{c_c=c.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",");out+='<div id="tipue_search_results_count">'+c_c+" результата(ов)</div>"}found.sort();var l_o=0;for(var i=0;i<found.length;i++){var fo=found[i].split("^");if(l_o>=
...
"_"+replace+'">'+(f+1)+"</a></li>"}else{var p_b=pages+2;if(p_b>pages)p_b=pages;for(var f=page;f<p_b;f++)if(f==page)out+='<li class="current">'+(f+1)+"</li>";else out+='<li><a href="javascript:void(0)" class="tipue_search_foot_box" id="'+f*set.show+"_"+replace+'">'+(f+1)+"</a></li>"}if(page+1!=pages)out+='<li><a href="javascript:void(0)" class="tipue_search_foot_box" id="'+(start+set.show)+"_"+replace+'">Next &#187;</a></li>';out+="</ul></div>"}}else out+='<div id="tipue_search_warning_head">Ничего не найдено</div>'}else if(show_stop)out+=
'<div id="tipue_search_warning_head">Ничего не найдено</div><div id="tipue_search_warning">Common words are largely ignored</div>';else{out+='<div id="tipue_search_warning_head">Слишком короткий запрос</div>';if(set.minimumLength==1)out+='<div id="tipue_search_warning">Должно быть не менее 1 символов</div>';else out+='<div id="tipue_search_warning">Должно быть '+set.minimumLength+" символа или больше</div>"}$("#tipue_search_content").html(out);$("#tipue_search_content").slideDown(200);$("#tipue_search_replaced").click(function(){getTipueSearch(0,
